...
class AccountContract implements Contract {
  ...
  ContractMaybeInit get initMaybe => ContractMaybeInit(
        code: init?.code,
        data: init?.data,
      );
  ...
  void Function(Builder builder) storeStateInit(StateInit init) =>
      (Builder builder) {
        builder.storeRef(init.code);
        builder.storeRef(init.data);
      };

  @override
  InternalAddress get address {
    final builder = beginCell();
    final stateInit = StateInit(null, null, init?.code, init?.data);
    final hash = builder.store(storeStateInit(stateInit)).endCell().hash();
    return InternalAddress(BigInt.zero, hash);
  }

  @override
  set address(InternalAddress address) {}

  @override
  ContractInit? get init {
    ...
    final system = Cell.fromBocBase64(
    ...
    builder.storeStringRefTail(accountOwner);
    final data = builder.endCell();
    return ContractInit(
      code: code,
      data: data,
    );
